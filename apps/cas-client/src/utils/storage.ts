...
class Storage {
  ...
  static async setItem<T>(
    ...
    localStorage.setItem(processedKey, processedValue)
  }

  static async removeItem(key: string): Promise<void> {
    localStorage.removeItem(Storage.createStorageKey(key))
  }
  ...
  static async getItem<T>(
    ...
    try {
      const data = localStorage.getItem(Storage.createStorageKey(key))
      if (data) {
