...
const ATTRIBUTES_TO_REMOVE: string[] = ['xmlns'];
...
export function generateIconsForSourceDir(sourceDir: string, destDir: string) {
  ...
  iconFiles.forEach((file: string) => {
    ...
    const reactNativeSvgSource: string =
      createReactNativeSvgSource(sourceFilePath);
...
function createReactNativeSvgSource(filePath: string): string {
  ...
  const reactNativeSvg: string =
    createReactNativeSvgElementFromSource(sourceSvg);

  const componentName = pascalCase(getFileNameFromPath(filePath));

  return createReactNativeElementSource(reactNativeSvg, componentName);
}

function createReactNativeElementSource(
  svg: string,
  ...
  return [
    ...
    `export const ${componentName} = (props?: Svg.SvgProps): React.ReactElement<Svg.SvgProps> => (`,
    svg,
    ');',
  ].join('\n');
}
...
function createReactNativeSvgElementFromSource(source: string): string {
  const withReplacedElements: string = replaceSourceSvgWithMap(
    source,
    ELEMENT_REPLACE_MAP
  );

  const cleanedFromUnnecessaryAttributes: string = removeUnnecessaryAttributes(
    withReplacedElements,
...
function removeUnnecessaryAttributes(
  ...
  return Object.values(attributes).reduce(
    (result: string, attribute: string): string => {
...
function replaceElementWithReactNativeSvgElement(
  source: string,
  element: string
): string {
  const replacementMap: ReplacementMap =
    createTagsReplacementMapForElement(element);

  return Object.keys(replacementMap).reduce(
    (result: string, replaceTag: string): string => {
